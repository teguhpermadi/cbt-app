...
class ExamResultDetailFactory extends Factory
{
    ...
    public function definition(): array
    {
        ...
        $studentAnswer = match ($questionType->value) {
            'multiple_choice' => $isCorrect ? $keyAnswer['answer'] : $this->faker->randomElement(['A', 'B', 'C', 'D']),
            'multiple_selection' => $isCorrect ? $keyAnswer['answers'] : array_merge($keyAnswer['answers'], [$this->faker->randomElement(['X', 'Y'])]) , // Jawaban salah/double
            'essay' => $this->faker->paragraph(3), // Jawaban panjang
            'ordering' => $isCorrect ? $keyAnswer['order'] : array_reverse($keyAnswer['order']),
            'numerical_input' => $isCorrect ? $keyAnswer['value'] : $keyAnswer['value'] + $this->faker->randomFloat(1, -2, 2),
            default => null,
        };
        ...
        return [
            ...
            'is_correct' => ($questionType->value === 'essay') ? null : $isCorrect, // Essay = null
            ...
            'time_spent' => rand(10, 90), // 10-90 detik per soal
        ];
    }
}