<div class="px-4 py-6 sm:px-6 lg:px-8">
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-4 gap-6 items-start">
            {{-- Header Data --}}
            <div class="xl:col-span-3 grid grid-cols-1 md:grid-cols-4 gap-4 text-sm bg-zinc-50 dark:bg-zinc-800/40 p-5 rounded-xl border border-zinc-200 dark:border-zinc-700 shadow-sm transition-colors">
                <div>
            ...
            <div id="question-{{ $index + 1 }}"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8 pt-8 border-t-2 border-zinc-50 dark:border-zinc-800/50">
                    ...
                    <div class="group">
                        ...
                        <div class="p-4 bg-zinc-50 dark:bg-zinc-950/50 rounded-xl border-2 border-zinc-100 dark:border-zinc-800 min-h-[60px] flex items-center">
                            ...
                            <ul class="w-full space-y-2">
                                ...
                                <li class="flex items-center justify-between p-2 bg-white dark:bg-zinc-900 rounded-lg border border-zinc-100 dark:border-zinc-800 text-xs">
                                    <span class="font-medium text-zinc-500">{{ $left }}</span>
                                    <x-heroicon-m-arrow-long-right class="w-4 h-4 text-zinc-300" />
                                    <span class="font-bold text-primary">{{ $right ?? '-' }}</span>
                                </li>
                                @endforeach
                            </ul>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <span class="px-3 py-1.5 bg-white dark:bg-zinc-900 rounded-lg text-xs font-bold border border-zinc-100 dark:border-zinc-800 shadow-sm text-zinc-700 dark:text-zinc-300">
                                    {{ $item }}
                                </span>
                                @endforeach
                            </div>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <span class="px-3 py-1.5 bg-primary/10 text-primary rounded-lg text-xs font-bold border border-primary/20 shadow-sm">
                                    {{ $ans }}
                                </span>
                                @endforeach
                            </div>
                    ...
                    <div class="group">
                        ...
                        <div class="p-4 bg-success/5 dark:bg-success/10 rounded-xl border-2 border-success/10 min-h-[60px] flex items-center">
                            @php
                            $type = $question->question_type;
                            @endphp

                            @if($type === \App\Enums\QuestionTypeEnum::MultipleChoice || $type === \App\Enums\QuestionTypeEnum::TrueFalse)
                            <span class="font-black text-success text-3xl">
                                {{ $keyAnswer['answer'] ?? '-' }}
                            </span>
                            @elseif($type === \App\Enums\QuestionTypeEnum::MultipleSelection)
                            <div class="flex flex-wrap gap-2">
                                @foreach($keyAnswer['answers'] ?? [] as $ans)
                                <span class="px-3 py-1.5 bg-success text-success-fg rounded-lg text-sm font-black shadow-sm">
                                    {{ $ans }}
                                </span>
                                @endforeach
                            </div>
                            ...
                            <ul class="w-full space-y-2">
                                ...
                                <li class="flex items-center justify-between p-2 bg-white/50 dark:bg-zinc-900/50 rounded-lg border border-success/20 text-xs text-zinc-900 dark:text-zinc-100">
                                    <span class="font-medium text-zinc-500">{{ $left }}</span>
                                    <x-heroicon-m-arrow-long-right class="w-4 h-4 text-success/40" />
                                    <span class="font-black text-success">{{ $right }}</span>
                                </li>
                                @endforeach
                            </ul>
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                <span class="px-3 py-1.5 bg-success/10 text-success rounded-lg text-sm font-black border border-success/20 shadow-sm">
                                    {{ $item }}
                                </span>
                                @endforeach
                            </div>
                            ...
                            <div class="flex items-baseline gap-1">
                                <span class="font-black text-success text-2xl">{{ $keyAnswer['answer'] ?? '-' }}</span>
