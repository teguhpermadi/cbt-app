    ...
    <div class="flex flex-col sm:flex-row sm:items-center space-y-2 sm:space-y-0 sm:space-x-3">
        <div class="flex items-center space-x-2">
            <div class="relative">
                ...
                >
                    <option value="">Pilih Timer</option>
                    @foreach($this->getTimerOptions() as $value => $label)
                        <option value="{{ $value }}" {{ $selectedTimer?->value == $value ? 'selected' : '' }}>
                            {{ $label }}
                        </option>
                    @endforeach
                </select>
                ...
                @if($isLoading)
                    <div class="absolute inset-0 flex items-center justify-center pointer-events-none bg-white/70 dark:bg-gray-800/70 rounded-lg">
                        <div class="flex items-center space-x-2">
                            ...
                            <span class="text-xs text-blue-600 dark:text-blue-400 font-medium">Loading...</span>
                        </div>
                    </div>
                @endif
            </div>            
